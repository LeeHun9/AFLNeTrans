...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           //transbitmap_changed = 1,   /* Time to update trans_bitmap?     */
...
u32 update_scores_and_select_next_state(u8 mode) {
  ...
  for(i = 0; i < state_ids_count; i++) {
    ...
    k = kh_get(hms, khms_states, state_id); //kh_get use to find the iterator of key( state_id: state_info)
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  for(i = 0; i < state_count; i++) {
    ...
    if (kh_get(hs32, khs_state_ids, state_id) != kh_end(khs_state_ids)) {
      ...
      kh_put(hs32, khs_state_ids, state_id, &discard);    // skip redundant state id
      k = kh_get(hms, khms_states, state_id);
      if (k != kh_end(khms_states)) {
...
int send_over_network()
{
  ...
  if (response_bytes) {   // clear this varity
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    // record each size of send's receive content, idx map to region id
...
// A clone from has_new_bits, to check if this seed has new state transition.
...
static void remove_shm(void) {
  ...
  //ck_free(trans_bits);      // free when exit
...
static void cull_queue(void) {
  ...
  while (q) {
    if (!q->is_initial_seed)    // initial seeds is important
      q->favored = 0;
    q = q->next;
  }
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {      // crash_mode is 0, fault == 0 meaning no fault
    ...
    // first: code coverage; second: trans_coverage
    // skip both no new cover
    if (!hnb && (!state_trans_fuzzing || !hnt)) {
      if (crash_mode) total_crashes++;
      return 0;
    }

    // if hnb == 0, but have more state trans(hnt == 1), 95% to skip this testcase
    //if (!hnb && (state_trans_fuzzing && hnt == 1) && UR(100) < SKIP_NFAV_OLD_PROB) {
    //  return 0;
    //}
    ...
    fn = alloc_printf("%s/queue/id:%06u,%s%s", out_dir, queued_paths,
                      describe_op(hnb), (state_trans_fuzzing && hnt)? ",+trans":"");
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  else if (q->uniq_trans * 3 > avg_trans_size) perf_score *= 0.25;
...
static u8 fuzz_one(char** argv) {
  ...
  // leehung: maybe shouldn't skip the favor selection
  ...
  if (pending_favored) {
    ...
    if ((queue_cur->was_fuzzed || !queue_cur->favored) &&
        UR(100) < SKIP_TO_NEW_PROB) return 1;   // 99% skip this seed if was_fuzzed = 1 or no favored
    ...
    if (target_state_id == 0) {
      ...
      for(i = 0; i < queue_cur->region_count; i++) {
        ...
        if (regionalStateCount > 0) {
          ...
          //No annotation for this region, queue_cur can't reach target id, return 1.
          return 1;
        }
      }

      //Then identify M2_region_count, by comparing region's state_counts 
    ...
    if (responses_temp != NULL) {
      ...
      if (!empty) {
        ...
        if (example_len > EXAMPLES_PROMPT_LENGTH) {
          ...
          char* example_temp = strdup(examples + offset);
          free(examples);
...
free_stall:
  ...
  if (M2_prev == NULL) {    // M1 is empty
    it = kl_begin(kl_messages);
  } else {                  // M1 is not empty
    it = kl_next(M2_prev);
  }

  // cpy contents between M2_prev and M2_next to in_buf
  u32 in_buf_size = 0;
  while (it != M2_next) {
  ...
  // cpy in_buf to out_buf to be mutated
  orig_in = in_buf;
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+Zi:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:")) > 0)
    ...
    switch (opt) {
      ...
      case 'Z':
        if (state_trans_fuzzing) FATAL("Multiple -Z options not supported");
        state_trans_fuzzing = 1;
        break;
